...
- [x] 2.2 Reemplazar el trigger de cierre por un indicador textual accesible de estado y mantener una vía explícita hacia el flujo de registrar cierre — prueba manual.
- [x] 2.3 Refrescar el indicador después de registrar un cierre y después de una venta confirmada sin mover el foco del escáner — prueba manual en POS.
- [x] 2.4 Resolver carga, error recuperable, éxito y pending sin bloquear el POS ni reintentar automáticamente — inspección y prueba manual.
- [x] 2.5 Verificar teclado, retorno de foco, móvil y que el estado no dependa sólo de color — prueba manual.

## 3. Reporte administrativo diario

- [x] 3.1 Crear una ruta Admin-only para el reporte de conciliación y agregar su acceso dentro de Reportes sin mostrarlo a Cashier ni Inventory — inspección y prueba manual de roles.
- [x] 3.2 Renderizar la tabla por cajero/día con estado, ventas, efectivo esperado, contado, diferencia y hora del último cierre — prueba manual.
- [x] 3.3 Implementar filtros de rango, loading, vacío y error con recuperación, usando los primitives y tokens existentes — prueba manual.
- [x] 3.4 Verificar tabla responsive, encabezados semánticos, cifras tabulares y copy en español — prueba manual.
...
- [x] 4.2 Probar contra backend real los estados En curso, Cierre registrado, Pendiente de actualizar, Sin cerrar y Sin actividad, incluidos `401` y `403`.
- [x] 4.3 Probar que registrar un cierre o confirmar una venta no bloquea ni degrada el flujo POS.
- [x] 4.4 Sincronizar specs y archivar sólo con decisión explícita posterior del usuario.
