...
**Reason**: `GET /api/v1/reports/stock/history` is removed by the backend's `add-reporting-v15`; ownership of stock movement data moves to Inventory.

**Migration**: The capability is not lost — it is specified in `ui-inventory` by `add-frontend-inventory-v15`, backed by `GET /api/v1/inventory/movements`, gaining pagination, a type filter, the acting user's name, and the previous/new quantity transition. Both changes must ship in the same release.
