...
The frontend SHALL render a navigation shell that shows only the sections permitted for the authenticated user's **roles**, where a user holds one or more roles and the visible sections are the **union** of what each role permits. Gating SHALL be evaluated as an intersection between the session's roles and the roles a section or route declares, never as equality against a single role. Sections by role: Cashier — POS and their own sales; Inventory Manager — products, stock, movements; Receiving — stock loading and the receiving section, without catalog write access; Admin — everything, including categories, users, reports and **the expenses section**. The expenses section SHALL be visible to Admin only, and no other role SHALL see it or reach any of its routes. Role gating remains UX only; the backend enforces.

#### Scenario: Cashier sees only sales navigation
- **WHEN** a user whose only role is `cashier` is authenticated
- **THEN** the navigation shows the POS section and does not show products, inventory, receiving, categories, reports or expenses
...
#### Scenario: Expenses section is Admin-only
- **WHEN** a user who does not hold `admin` is authenticated
- **THEN** the navigation does not show the expenses section, and navigating directly to any expenses route redirects them away without requesting expense data

#### Scenario: Admin sees the expenses section
- **WHEN** a user holding `admin` is authenticated
- **THEN** the navigation lists the expenses section and opening it renders the expenses hub

#### Scenario: Union of two roles
- **WHEN** a user holds `cashier` and `receiving`
- **THEN** the navigation shows the POS section, their sales, the receiving section and stock loading, and still does not show categories, users, reports or expenses
