...
### Requirement: Payment composition
The cashier SHALL compose one or more payments for a sale, each with a method (cash or card) and an amount, submitted as the complete list to `PUT /api/v1/sales/{id}/payment`. Choosing a single method SHALL remain a single action that imputes the full sale total to that method, requiring no amount entry; splitting SHALL be an optional secondary action that is discoverable from the payment area. When amounts are split, the cashier SHALL enter one amount and the remaining amount SHALL be derived by decimal subtraction from the total — never by division, proportional split, or any operation that can round. All monetary arithmetic SHALL use decimal-safe helpers and SHALL NOT use floating point.

#### Scenario: Single method stays one action
- **WHEN** the cashier selects cash or card without splitting
