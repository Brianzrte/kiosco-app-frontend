...
- [ ] 0.1 Bloqueado por `backend-request.md`: verificar contra una instancia
...
- [ ] 0.2 Bloqueado por `backend-request.md`: verificar contra una instancia
      ...
      esto.
- [ ] 0.3 Confirmar con el backend los nombres definitivos de los campos
...
- [ ] 0.4 Resolver el orden de merge con el change abierto
...
- [ ] 5.3 Al escanear de nuevo un producto `pesable` ya presente en el
      ...
      `design.md`).
- [ ] 5.4 Calcular y mostrar el precio de la línea `pesable` con el helper
      de la sección 2 apenas el peso es válido.
- [ ] 5.5 Agregar el control de edición de precio real (botón con ícono
...
- [ ] 5.6 Usar el precio efectivo (helper 2.2) de cada línea para el total
...
- [ ] 5.7 No consultar ni cap-ear contra `GET /inventory/stock/{product_id}`
...
- [ ] 5.8 Rechazar inline (sin llamar al backend) un peso negativo, no
      numérico o con más de 3 decimales, reutilizando el helper de la
      sección 2.
- [ ] 5.9 Rechazar la incorporación de un producto `pesable` sin
      `price_per_kg` válido, con el mismo tratamiento visual que un producto
      inactivo hoy.
- [ ] 5.10 Enviar, por cada línea `pesable`, su peso y — cuando exista — su
      precio real en `POST /sales/{id}/items`, en el mismo recorrido que hoy
      hace `confirmSale()` sobre `cart`.
- [ ] 5.11 Verificar que el foco vuelve al input de escaneo después de
      abrir/cerrar el editor de precio real, igual que el resto de acciones
      del carrito (`refocus()`).
- [ ] 5.12 Prueba manual: mezclar productos `unitario` y `pesable` en el
...
- [ ] 7.1 Ejecutar `npm run lint` y corregir hallazgos.
- [ ] 7.2 Ejecutar `npm test` y corregir fallos; confirmar que los tests
      nuevos de la sección 2.4 pasan.
- [ ] 7.3 Ejecutar `npm run build` por el impacto en tipos, `ProductForm`,
      `PosView`, `SaleDetail` y `ProductsView`.
- [ ] 7.4 Validar contra backend real la creación de productos `pesable`, la
...
- [ ] 7.5 Ejecutar la revisión UX/UI siguiendo
...
- [ ] 8.1 Revisar que el diff sólo contenga el alcance implementado, sin
      dependencias nuevas ni llamadas directas al backend fuera de `api()`.
- [ ] 8.2 Sincronizar los deltas con `openspec/specs/ui-catalog/spec.md`,
